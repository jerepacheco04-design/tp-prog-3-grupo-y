    ...
    internal class ClasificacionDAO
    {
        ...
        internal List<KeyValuePair<int, string>> Listar()
        {
            ...
            using (var dr = cmd.ExecuteReader())
                while (dr.Read())
                    lista.Add(new KeyValuePair<int, string>((int)dr["Id"], Convert.ToString(dr["Descripcion"])));
            return lista;
        }
        ...
        internal int Guardar(int id, string descripcion, bool nuevo)
        {
            ...
            if (!nuevo)
                ...
                throw new ArgumentException("Un registro nuevo debe tener Id cero.");
            using (var cn = datos.AbrirConexion())
            using (var tx = cn.BeginTransaction(IsolationLevel.Serializable))
            {
                using (var cmd = new SqlCommand("SELECT COUNT(*) FROM " + tabla + " WITH (UPDLOCK,HOLDLOCK) WHERE Descripcion=@Descripcion AND Id<>@Id", cn, tx))
                {
                    ...
                    if ((int)cmd.ExecuteScalar() > 0)
                        throw new ArgumentException("Ese nombre ya existe.");
                }

                string sql = nuevo ? "INSERT INTO " + tabla + " (Descripcion) VALUES (@Descripcion); SELECT CAST(SCOPE_IDENTITY() AS int);" : "UPDATE " + tabla + " SET Descripcion=@Descripcion WHERE Id=@Id";
