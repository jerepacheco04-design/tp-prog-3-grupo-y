...
namespace TPWinForm_equipo_X
{
    public static class ImagenRemota
    {
        public static string Normalizar(string enlace)
        {
            ...
            {
                Uri foto;
                if(Uri.TryCreate(parametros["imgurl"],UriKind.Absolute,out foto) && (foto.Scheme=="http" || foto.Scheme=="https")) uri=foto;
            }
        ...
        public static async Task<Bitmap> Cargar(string enlace,CancellationToken token)
        {
            ...
            {
                try { return ArchivosImagen.LeerArchivo(archivo); }
            ...
            using(var limite=CancellationTokenSource.CreateLinkedTokenSource(token))
            {
                ...
                using(var respuesta=await cliente.GetAsync(url,HttpCompletionOption.ResponseHeadersRead,limite.Token).ConfigureAwait(false))
                {
                    ...
                    if(tipo=="text/html") throw new InvalidOperationException("Ese enlace abre una página, no una foto. En el navegador elegí Copiar dirección de imagen.");
                    if(respuesta.Content.Headers.ContentLength>ArchivosImagen.MaximoBytes) throw new ArgumentException("La foto supera 15 MB.");
                    using(var origen=await respuesta.Content.ReadAsStreamAsync().ConfigureAwait(false))
                    using(var memoria=new MemoryStream())
                    {
                        byte[] buffer=new byte[8192]; int cantidad;
                        while((cantidad=await origen.ReadAsync(buffer,0,buffer.Length,limite.Token).ConfigureAwait(false))>0)
                        {
                            if(memoria.Length+cantidad>ArchivosImagen.MaximoBytes) throw new ArgumentException("La foto supera 15 MB.");
                        ...
                        try
                        {
                            // La caché nunca modifica el enlace ni los datos del artículo.
                            ...
                            try
                            {
                                ...
                                if(!File.Exists(archivo)) File.Move(temporal,archivo);
                            }
