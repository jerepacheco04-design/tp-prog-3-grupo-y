...
namespace TPWinForm_equipo_X
{
    public sealed class FotoArticulo
    {
        ...
        public override string ToString() { return Nombre ?? Referencia; }
    }
    ...
    public static class ArchivosImagen
    {
        ...
        public static string CarpetaRaiz
        {
            get
            {
                ...
                while(carpeta != null)
                {
                    if(File.Exists(Path.Combine(carpeta.FullName,"TPWinForm_equipo-X.sln"))) return carpeta.FullName;
        ...
        {
            if(!RutaImagen.EsLocal(referencia)) throw new ArgumentException("La referencia de la foto local no es válida.");
        ...
        public static void ValidarArchivo(string archivo)
        {
            ...
            if(!info.Exists) throw new FileNotFoundException("No se encontró la foto seleccionada.");
            if(info.Length > MaximoBytes) throw new ArgumentException("Cada foto puede ocupar hasta 15 MB.");
            using(var foto=LeerArchivo(archivo)) { }
        }
        public static Bitmap LeerArchivo(string archivo)
        {
            using(var stream=File.OpenRead(archivo)) return Leer(stream);
        }
        public static Bitmap Leer(Stream stream)
        {
            ...
            try
            {
                var decodificador=System.Windows.Media.Imaging.BitmapDecoder.Create(stream,
                    System.Windows.Media.Imaging.BitmapCreateOptions.PreservePixelFormat,
                    System.Windows.Media.Imaging.BitmapCacheOption.OnLoad);
                var cuadro=decodificador.Frames[0];
                if((long)cuadro.PixelWidth*cuadro.PixelHeight>40000000) throw new ArgumentException("La foto supera 40 megapíxeles.");
                var encoder=new System.Windows.Media.Imaging.PngBitmapEncoder();
                encoder.Frames.Add(System.Windows.Media.Imaging.BitmapFrame.Create(cuadro));
                using(var memoria=new MemoryStream())
                {
                    encoder.Save(memoria); memoria.Position=0;
                    using(var foto=Image.FromStream(memoria,true,true)) return new Bitmap(foto);
                }
            }
            catch(NotSupportedException) { throw new ArgumentException("El archivo no es una imagen compatible. Probá JPG, PNG o WebP (requiere el códec de Windows)."); }
        }
        public static string Importar(string archivo)
        {
            ...
            try { File.Copy(archivo,destino,false); }
            catch { if(File.Exists(destino)) File.Delete(destino); throw; }
            return referencia;
        }
        public static void DescartarCopiaNueva(string referencia)
        {
            // Solo se llama para copias nuevas de un guardado que falló, nunca para fotos existentes.
